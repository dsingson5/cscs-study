...
document.addEventListener("DOMContentLoaded", () => {
  injectSyncUI();
  if (syncEnabled()) {
    // __didHardReset now means exactly one thing: a deliberate RESET_EPOCH wipe
    // just ran. (The qbank migration used to set it too, which is how an
    // accidental wipe reached the cloud.) A deliberate wipe MUST push — gating
    // this on "do we still have cards" would never fire, since maybeHardReset()
    // empties them first, and syncPull would just haul the wiped data back.
    if (__didHardReset) syncPush({ silent: true }).catch(() => {});
...
/* ===== study-voice v1 — listen buttons (Web Speech) + NotebookLM helpers.
   Synced copies live in hyrox_study/app.js and cscs_study/app.js — keep both
   in step. No external services: speech uses the browser's own voices, and the
   NotebookLM buttons prepare copy-paste material (NotebookLM has no public
   API, so nothing can be generated for the user directly). ===== */
(function () {
  "use strict";
  ...
  onReady(function () {
    // Only lesson-bearing pages (daily pages + the entry copy). Dashboards,
    // galleries and games pages don't get the panel.
    if (!document.querySelector(".lesson-card") && !document.querySelector(".q")) return;

    var synth = window.speechSynthesis || null;

    // ── prefs ────────────────────────────────────────────────────────────
    var prefs = { accent: "gb", rate: 1, answers: "revealed", voiceName: "" };
    try {
      var raw = localStorage.getItem(CFG.prefix);
      if (raw) { var p = JSON.parse(raw); for (var k in prefs) if (p[k] !== undefined) prefs[k] = p[k]; }
    } catch (e) {}
    function savePrefs() { try { localStorage.setItem(CFG.prefix, JSON.stringify(prefs)); } catch (e) {} }

    // ── voices ───────────────────────────────────────────────────────────
    // Ranked, best-first. Edge exposes the excellent "Online (Natural)"
    // voices; Chrome exposes Windows SAPI + Google network voices; iOS has
    // Daniel/Arthur (GB) and Aaron/Alex (US).
    var GB = ["ryan", "thomas", "george", "daniel", "arthur", "oliver",
              "google uk english male", "brian"];
    var US = ["andrew", "guy", "christopher", "roger", "eric", "davis",
              "microsoft david", "microsoft mark", "alex", "aaron"];
    var FEMALEISH = /female|zira|hazel|susan|sonia|libby|aria|jenny|michelle|emma|ava|salli|joanna|kate|serena|martha|google us english$/i;

    function voiceScore(v, accent) {
      var name = (v.name || "").toLowerCase();
      var lang = (v.lang || "").toLowerCase().replace("_", "-");
      var want = accent === "gb" ? "en-gb" : "en-us";
      var list = accent === "gb" ? GB : US;
      var s = 0;
      if (lang === want) s += 60;
      else if (lang.indexOf("en") === 0) s += 15;
      else return -1;
      for (var i = 0; i < list.length; i++)
        if (name.indexOf(list[i]) >= 0) { s += 200 - i * 10; break; }
      if (/natural|online/.test(name)) s += 25;
      if (FEMALEISH.test(name)) s -= 40;
      return s;
    }
    ...
    // ── speech engine ────────────────────────────────────────────────────
    var queue = [], qi = 0, speaking = false, paused = false, currentEl = null, gen = 0;
    ...
    // Split into sentence-ish chunks ≤ ~220 chars: long single utterances are
    // where Chrome's speech engine stalls.
    function chunk(text) {
    ...
    function playItems(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        var chunks = chunk(items[i].text);
        for (var c = 0; c < chunks.length; c++)
          queue.push({ text: chunks[c], el: items[i].el });
      }
      if (!queue.length) { toast("Nothing to read here yet."); return; }
      qi = 0; speaking = true; paused = false;
      updatePanel();
      // Chrome drops the first utterance if speak() follows cancel() at once.
      setTimeout(speakNext, 80);
    }

    function stopAll() {
      gen++;
      speaking = false; paused = false; queue = []; qi = 0;
      if (synth) try { synth.cancel(); } catch (e) {}
      clearHighlight(); updatePanel();
    }
    ...
    // Un-stick Chrome's engine on long reads; resume() while speaking and not
    // user-paused is a no-op elsewhere.
    setInterval(function () {
      if (synth && speaking && !paused && synth.speaking) { try { synth.resume(); } catch (e) {} }
    }, 10000);
    ...
    // Full text WITH answers — source material for NotebookLM.
    function sourceText() {
    ...
    function nlmPrompt(mode) {
      ...
      if (mode === "video")
        return 'Create a cinematic Video Overview of "' + t + '". Style: ' +
          ...
          "visual scene per key fact, closing with a single training-floor application. " +
      ...
      return 'Create a detailed infographic of "' + t + '". Art direction: kawaii ' +
        ...
        "glossary terms as a labelled icon strip, and one 'training-floor application' " +
    ...
    function copyText(s, btn, okLabel) {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(s).then(function () { done(true); },
          function () { legacyCopy(s) ? done(true) : done(false); });
    ...
    function toast(msg) {
      ...
      setTimeout(function () { t.classList.remove("on");
        setTimeout(function () { t.remove(); }, 300); }, 2200);
    }

    // ── UI: floating panel ───────────────────────────────────────────────
    var panel, btnPlay, btnPause, voiceSel;
    ...
    function fillVoiceSelect() {
      ...
      auto.value = ""; auto.textContent = "Voice: automatic (best available)";
      voiceSel.appendChild(auto);
      for (var i = 0; i < ranked.length; i++) {
        var o = document.createElement("option");
        o.value = ranked[i].v.name;
        o.textContent = ranked[i].v.name.replace(/^Microsoft /, "").replace(/ - English.*$/, "");
    ...
    function buildPanel() {
      ...
      btnPlay = mkBtn("🔊 Listen to page", "Read the whole lesson aloud", "sv-main",
        function () { playItems(pageItems()); });
      btnPause = mkBtn("⏸ Pause", "Pause / resume", "", togglePause);
      var btnStop = mkBtn("⏹", "Stop", "sv-stop", stopAll);
      row1.appendChild(btnPlay); row1.appendChild(btnPause); row1.appendChild(btnStop);

      var row2 = document.createElement("div"); row2.className = "sv-row";
      var gb = mkBtn("🇬🇧 British", "Deep British male voice — documentary style", "sv-seg",
        function () { prefs.accent = "gb"; prefs.voiceName = ""; savePrefs(); fillVoiceSelect(); updatePanel(); });
      gb.setAttribute("data-accent", "gb");
      var us = mkBtn("🇺🇸 American", "American male voice", "sv-seg",
        function () { prefs.accent = "us"; prefs.voiceName = ""; savePrefs(); fillVoiceSelect(); updatePanel(); });
      ...
      voiceSel.className = "sv-select";
      voiceSel.title = "Exact voice (from this device's installed voices)";
      voiceSel.addEventListener("change", function () {
        prefs.voiceName = voiceSel.value; savePrefs();
      });
      ...
      row3.appendChild(voiceSel); row3.appendChild(ansSel);

      var row4 = document.createElement("div"); row4.className = "sv-row";
      var lab = document.createElement("span");
      lab.className = "sv-lab"; lab.textContent = "NotebookLM:";
      row4.appendChild(lab);
      row4.appendChild(mkBtn("🎬 Video", "Prepare a cinematic NotebookLM Video Overview", "",
        function () { openNlm("video"); }));
      row4.appendChild(mkBtn("🎙️ Podcast", "Prepare a long NotebookLM Deep Dive podcast", "",
        function () { openNlm("podcast"); }));
      row4.appendChild(mkBtn("📊 Infographic", "Prepare a kawaii-powers NotebookLM infographic", "",
        function () { openNlm("infographic"); }));

      panel.appendChild(row1); panel.appendChild(row2);
      panel.appendChild(row3); panel.appendChild(row4);
    ...
    function openNlm(mode) {
      ...
      h.textContent = "Create " + (/^[aeiou]/i.test(NLM_LABEL[mode]) ? "an " : "a ") + NLM_LABEL[mode] + " with NotebookLM";
      ...
      var open = mkBtn("Open NotebookLM ↗", "notebooklm.google.com", "sv-main",
        function () { window.open("https://notebooklm.google.com/", "_blank", "noopener"); });
      var close = mkBtn("Close", "", "", function () { ov.remove(); });
    ...
    buildPanel();
    injectButtons();
    // Review-queue cards render late (after the async FSRS module) and the
    // container is rebuilt after every grade — re-inject when that happens.
